...
这个包是 `@uni-helper/uni-typed` 项目的一部分，将 uni-app 内置组件包装为可导入的 Vue v3 组件，并提供对应的 TypeScript 实例类型。

与 [`@uni-helper/uni-app-types`](./uni-app-types.md) 不同，这个包允许你在 Vue SFC 中以 PascalCase 直接导入并使用组件，而无需依赖模板的全局自动识别。

::: tip

如果你只希望在 Template 里获得类型提示，安装 [`@uni-helper/uni-app-types`](./uni-app-types.md) 即可，无需安装这个包。
...
import { Button, View } from '@uni-helper/uni-app-components';
</script>
...
import UniAppComponents from '@uni-helper/uni-app-components';

export function createApp() {
  const app = createSSRApp(App);
  app.use(UniAppComponents);
  return { app };
}
```
...
<script setup lang="ts">
import { ref } from 'vue';
import { ScrollView, type ScrollViewInstance } from '@uni-helper/uni-app-components';
...
请查看 [源代码](https://github.com/uni-helper/uni-typed/tree/main/packages/uni-app-components/src/components) 了解所有组件。
